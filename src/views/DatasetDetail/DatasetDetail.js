  ...
  render() {
    ...
    return isFetching === true ? <h1 className="text-center p-5"><i className="fas fa-circle-notch fa-spin mr-2" />Caricamento</h1> : (<div>
      {(ope === 'RECEIVE_DATASET_DETAIL' || ope === 'RECEIVE_FILE_STORAGEMANAGER') && (dataset) &&
        <div>
          <div className='top-dataset pt-4'>
            <div className="container pl-0">
              <i className="fa fa-table fa-lg icon-dataset pr-2 fa-pull-left text-primary"></i>
              <h2 className="title-dataset text-primary text-truncate" title={dataset.dcatapit.title}>{this.truncate(dataset.dcatapit.title, 75)}</h2>
              <ul className="pl-4 nav nav-tabs b-b-0 buttons-nav">
              ...
              <button className="btn btn-accento buttons-nav" style={{ right: '24%', height: '48px' }} onClick={this.handleDownloadFile.bind(this, dataset.dcatapit.name, dataset.operational.logical_uri)}>Download {this.state.downloadState === 4 ? <i className="ml-4 fa fa-spinner fa-spin" /> : <i className="ml-4 fa fa-download" />}</button>
            </div>
          </div>
          ...
          <div hidden={!this.state.showDett} className="w-100 bg-light">
          ...
          <div hidden={!this.state.showWidget} className="w-100 card-text pt-4 bg-light">
      ...
      {(ope === 'RECEIVE_METADATA' && metadata) &&
        <div>
          <div className='top-dataset pt-4'>
            <div className="container">
              <i className="fa fa-table fa-lg icon-dataset pr-3 fa-pull-left text-primary"></i>
              <h2 className="title-dataset text-primary text-truncate" title={metadata.title}>{this.truncate(metadata.title, 75)}</h2>
              <ul className="pl-4 nav nav-tabs b-b-0 w-100 buttons-nav">
                <li className="nav-item">
                ...
                <div className="row px-3">
                  <div className="col-12 card-text" >
                    {metadata.resources && metadata.resources.map((res, index) => {
                      ...
                      return (
                        <div className="row b-b-1" key={index}>
                          <div className="col-8 py-4">
                            <div className="text-muted" key={index}>
                              <i className="fa-pull-left text-icon fa fa-sticky-note fa-lg mr-3 mt-1" style={{ lineHeight: '1' }} /><h4 className="mb-3"><b>{res.name}</b></h4>
                            </div>
                            <div>
                              <div className="">
